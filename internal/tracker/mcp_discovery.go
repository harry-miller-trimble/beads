...
// them as tracker driver factories in the global registry. Only plugins with
// tracker.read grants are registered.
//
// This is how external tracker drivers (ADO, Jira, etc.) are loaded at startup.
// Each driver is a separate binary that speaks MCP over stdio, satisfying the
// IssueTracker interface across process boundaries. The trust layer verifies
// the driver binary's digest before it will be started.
//
