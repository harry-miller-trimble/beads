...
func TestTracker_InitFromConfig(t *testing.T) {
	// TODO: adapt for standalone driver — config store tests no longer applicable.
	// Init now reads from env vars only. Test env-var-based init instead.
	t.Setenv("AZURE_DEVOPS_PAT", "config-pat")
	t.Setenv("AZURE_DEVOPS_ORG", "configorg")
	t.Setenv("AZURE_DEVOPS_PROJECT", "configproject")
...
func TestTracker_InitMissingProject(t *testing.T) {
	...
	t.Setenv("AZURE_DEVOPS_ORG", "myorg")
	t.Setenv("AZURE_DEVOPS_PROJECT", "")
	t.Setenv("AZURE_DEVOPS_PROJECTS", "")
	tr := &Tracker{}
	err := tr.Init(context.Background())
	if err == nil {
...
// TODO: adapt for standalone driver — custom state/type mappings are not yet
// supported in env-var-only mode.
...
// TODO: adapt for standalone driver — custom type mappings not supported yet.
func TestTracker_InitWithCustomTypeMapping(t *testing.T) {
	t.Skip("custom type mappings via config store not available in standalone driver")
}
...
func TestTracker_BuildExternalRef(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tracker: &Tracker{org: "myorg", projects: []string{"myproj"}},
			issue:   &TrackerIssue{Identifier: "99"},
		...
		},
		{
			...
			issue:   &TrackerIssue{Identifier: "66"},
...
// TODO: adapt for standalone driver — registration via tracker.Register removed
func TestTracker_Registration(t *testing.T) {
	t.Skip("tracker registration not applicable to standalone driver")
}
...
// TODO: adapt for standalone driver — getConfig used storage which is removed.
// Init now reads env vars directly. Test env-var precedence through Init instead.
func TestTracker_GetConfig_Precedence(t *testing.T) {
	t.Skip("getConfig removed in standalone driver — Init reads env vars directly")
}
...
func TestTracker_FetchIssues_APIError(t *testing.T) {
	...
	tr, _ := newTestTracker(t, mux)
	_, err := tr.FetchIssues(context.Background(), FetchOptions{})
	if err == nil {
...
func TestTracker_CreateIssue_APIError(t *testing.T) {
	...
	tr, _ := newTestTracker(t, mux)
	_, err := tr.CreateIssue(context.Background(), &BeadsIssue{
		Title:     "Will Fail",
		IssueType: TypeTask,
	})
	if err == nil {
...
func TestTracker_UpdateIssue_InvalidID(t *testing.T) {
	...
	_, err := tr.UpdateIssue(context.Background(), "abc", &BeadsIssue{Title: "x"})
	if err == nil {
...
func TestTracker_UpdateIssue_APIError(t *testing.T) {
	...
	tr, _ := newTestTracker(t, mux)
	_, err := tr.UpdateIssue(context.Background(), "999", &BeadsIssue{Title: "x"})
	if err == nil {
...
func TestTracker_UpdateIssue_ZeroID(t *testing.T) {
	...
	_, err := tr.UpdateIssue(context.Background(), "0", &BeadsIssue{Title: "x"})
	if err == nil {
...
func TestTracker_UpdateIssue_NegativeID(t *testing.T) {
	...
	_, err := tr.UpdateIssue(context.Background(), "-1", &BeadsIssue{Title: "x"})
	if err == nil {
...
func TestTracker_InitValidatesOrg(t *testing.T) {
	t.Setenv("AZURE_DEVOPS_PAT", "some-pat")
	t.Setenv("AZURE_DEVOPS_ORG", "bad org!@#")
	t.Setenv("AZURE_DEVOPS_PROJECT", "myproject")
	tr := &Tracker{}
	err := tr.Init(context.Background())
	if err == nil {
...
func TestTracker_InitValidatesProject(t *testing.T) {
	t.Setenv("AZURE_DEVOPS_PAT", "some-pat")
	t.Setenv("AZURE_DEVOPS_ORG", "myorg")
	t.Setenv("AZURE_DEVOPS_PROJECT", "bad<project>")
	tr := &Tracker{}
	err := tr.Init(context.Background())
	if err == nil {
...
func TestTracker_InitRejectsHTTPURL(t *testing.T) {
	...
	t.Setenv("AZURE_DEVOPS_ORG", "")
	t.Setenv("AZURE_DEVOPS_PROJECT", "myproject")
	t.Setenv("AZURE_DEVOPS_URL", "http://ado.example.com/collection")
	tr := &Tracker{}
	err := tr.Init(context.Background())
	if err == nil {
