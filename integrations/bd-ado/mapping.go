...
func (m *adoFieldMapper) IssueToBeads(ti *TrackerIssue) *IssueConversion {
	if ti == nil {
		return nil
	}
	...
	if issue.Status == StatusInProgress && hasBeadsTag(wi.GetStringField(FieldTags), "beads:blocked") {
	...
	if len(meta) > 0 {
		...
		if err == nil {
			...
			_ = json.Unmarshal(raw, &issue.Metadata)
		}
	}

	return &IssueConversion{Issue: issue, Dependencies: ExtractLinkDeps(wi)}
}

// IssueToTracker converts a beads Issue to a map of ADO work item field values.
func (m *adoFieldMapper) IssueToTracker(issue *BeadsIssue) map[string]interface{} {
	...
	if issue.Priority == 3 || issue.Priority == 4 {
		...
		issue.Metadata["beads_priority"] = strconv.Itoa(issue.Priority)
	}
...
func restoreMetadata(issue *BeadsIssue, fields map[string]interface{}) {
	if len(issue.Metadata) == 0 {
		return
	}
	if v, ok := issue.Metadata["ado.area_path"]; ok {
		fields[FieldAreaPath] = v
	}
	if v, ok := issue.Metadata["ado.iteration_path"]; ok {
		fields[FieldIterationPath] = v
	}
	if v, ok := issue.Metadata["ado.story_points"]; ok {
		fields[FieldStoryPoints] = v
	}
	if v, ok := issue.Metadata["ado.severity"]; ok {
