...
func TestStatusToBeads_Defaults(t *testing.T) {
	...
	tests := []struct {
		...
		{"New → open", "New", StatusOpen},
		{"Active → in_progress", "Active", StatusInProgress},
		{"Removed → deferred", "Removed", StatusDeferred},
		{"Closed → closed", "Closed", StatusClosed},
		{"Resolved → closed", "Resolved", StatusClosed},
		{"empty → open", "", StatusOpen},
		{"unknown → open", "CustomState", StatusOpen},
	}
...
func TestStatusToBeads_NonStringInput(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil → open", nil, StatusOpen},
		{"int → open", 42, StatusOpen},
		{"float → open", 3.14, StatusOpen},
		{"bool → open", true, StatusOpen},
	}
...
func TestStatusToTracker_DefaultCase(t *testing.T) {
	...
	got := m.StatusToTracker(string("unknown_status"))
	if got != "New" {
...
func TestStatusToTracker_CustomMapOverridesDefault(t *testing.T) {
	...
	tests := []struct {
		...
		{"custom open → To Do", StatusOpen, "To Do"},
		{"custom in_progress → Doing", StatusInProgress, "Doing"},
		{"custom closed → Finished", StatusClosed, "Finished"},
		{"custom blocked → On Hold", StatusBlocked, "On Hold"},
		{"custom deferred → Parked", StatusDeferred, "Parked"},
	}
...
func TestTypeToBeads_NonStringInput(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil → task", nil, TypeTask},
		{"int → task", 42, TypeTask},
		{"bool → task", true, TypeTask},
	}
...
func TestTypeToBeads_UnknownWithDefault(t *testing.T) {
	...
	if got != TypeTask {
		t.Errorf("TypeToBeads(CustomWorkItemType) = %q, want %q", got, TypeTask)
	}
}
...
func TestTypeToBeads_CustomMap(t *testing.T) {
	...
	tests := []struct {
		...
		{"custom PBI → feature", "Product Backlog Item", TypeFeature},
		{"custom Defect → bug", "Defect", TypeBug},
		{"custom case-insensitive", "product backlog item", TypeFeature},
		{"fallthrough Task → task", "Task", TypeTask},
	}
...
func TestTypeToTracker_DefaultCase(t *testing.T) {
	...
	got := m.TypeToTracker(string("unknown_type"))
	if got != "Task" {
...
func TestTypeToTracker_CustomMapAllTypes(t *testing.T) {
	...
	tests := []struct {
		...
		{"custom bug → Defect", TypeBug, "Defect"},
		{"custom feature → PBI", TypeFeature, "Product Backlog Item"},
		{"custom task → Work Item", TypeTask, "Work Item"},
		{"custom epic → Initiative", TypeEpic, "Initiative"},
		{"custom chore → Maintenance", TypeChore, "Maintenance"},
	}
...
func TestStatusToBeads_CustomMap(t *testing.T) {
	...
	tests := []struct {
		...
		{"custom Doing → in_progress", "Doing", StatusInProgress},
		{"custom Finished → closed", "Finished", StatusClosed},
		{"fallthrough New → open", "New", StatusOpen},
	}
...
func TestStatusToTracker_Defaults(t *testing.T) {
	...
	tests := []struct {
		...
		{"open → New", StatusOpen, "New"},
		{"in_progress → Active", StatusInProgress, "Active"},
		{"blocked → Active", StatusBlocked, "Active"},
		{"deferred → Removed", StatusDeferred, "Removed"},
		{"closed → Closed", StatusClosed, "Closed"},
	}
...
func TestStatusToTracker_CustomMap(t *testing.T) {
	...
	tests := []struct {
		...
		{"custom in_progress → Doing", StatusInProgress, "Doing"},
		{"custom closed → Finished", StatusClosed, "Finished"},
		{"fallthrough open → New", StatusOpen, "New"},
	}
...
func TestTypeToBeads_Defaults(t *testing.T) {
	...
	tests := []struct {
		...
		{"Bug → bug", "Bug", TypeBug},
		{"User Story → feature", "User Story", TypeFeature},
		{"Task → task", "Task", TypeTask},
		{"Epic → epic", "Epic", TypeEpic},
		{"bug lowercase → bug", "bug", TypeBug},
		{"user story lowercase → feature", "user story", TypeFeature},
		{"empty → task", "", TypeTask},
	}
...
func TestTypeToBeads_Scrum(t *testing.T) {
	...
	if got != TypeFeature {
		t.Errorf("TypeToBeads(Product Backlog Item) = %q, want %q", got, TypeFeature)
	}
}
...
func TestTypeToTracker_Defaults(t *testing.T) {
	...
	tests := []struct {
		...
		{"bug → Bug", TypeBug, "Bug"},
		{"feature → User Story", TypeFeature, "User Story"},
		{"task → Task", TypeTask, "Task"},
		{"epic → Epic", TypeEpic, "Epic"},
		{"chore → Task", TypeChore, "Task"},
	}
...
func TestTypeToTracker_CustomMap(t *testing.T) {
	...
	got := m.TypeToTracker(TypeFeature)
...
func TestIssueToBeads(t *testing.T) {
	...
	if issue.Status != StatusBlocked {
		t.Errorf("Status = %q, want %q", issue.Status, StatusBlocked)
	}
	if issue.IssueType != TypeBug {
		t.Errorf("IssueType = %q, want %q", issue.IssueType, TypeBug)
	}
	if issue.Assignee != "Alice Smith" {
		t.Errorf("Assignee = %q, want %q", issue.Assignee, "Alice Smith")
	}
	...
	if issue.ExternalRef != wantRef {
		t.Errorf("ExternalRef = %s, want %s", issue.ExternalRef, wantRef)
	}
...
func TestRestoreMetadata(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "only area_path",
			metadata: map[string]interface{}{"ado.area_path": "Project\\Team"},
			wantFields: map[string]interface{}{
		...
		},
		{
			...
			metadata: map[string]interface{}{"ado.iteration_path": "Sprint 1"},
			wantFields: map[string]interface{}{
		...
		},
		{
			...
			metadata: map[string]interface{}{"ado.story_points": float64(5)},
			wantFields: map[string]interface{}{
		...
		},
		{
			...
			metadata: map[string]interface{}{"ado.area_path": "A", "ado.iteration_path": "B", "ado.story_points": float64(8)},
			wantFields: map[string]interface{}{
				FieldAreaPath:      "A",
		...
		},
		{
			...
			metadata:   map[string]interface{}{"ado.rev": float64(3), "custom_field": "value"},
			wantFields: map[string]interface{}{},
		},
		{
			name:     "severity metadata restored",
			metadata: map[string]interface{}{"ado.severity": "2 - High"},
			wantFields: map[string]interface{}{
		...
		},
		{
			...
			metadata: map[string]interface{}{"ado.area_path": "A", "ado.iteration_path": "B", "ado.story_points": float64(8), "ado.severity": "1 - Critical"},
			wantFields: map[string]interface{}{
				FieldAreaPath:      "A",
				FieldIterationPath: "B",
				FieldStoryPoints:   float64(8),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			issue := &BeadsIssue{Metadata: tt.metadata}
			fields := map[string]interface{}{}
...
func TestBlockedStatusRoundTrip(t *testing.T) {
	...
	ti := &TrackerIssue{ID: "99", Raw: wi}
	conv := m.IssueToBeads(ti)
	if conv == nil {
		t.Fatal("pull: IssueToBeads returned nil")
	}
	if conv.Issue.Status != StatusBlocked {
		t.Errorf("pull: Status = %q, want %q", conv.Issue.Status, StatusBlocked)
	}
...
func TestActiveWithoutBlockedTag_StaysInProgress(t *testing.T) {
	...
	ti := &TrackerIssue{ID: "100", Raw: wi}
	conv := m.IssueToBeads(ti)
	if conv.Issue.Status != StatusInProgress {
		t.Errorf("Status = %q, want %q", conv.Issue.Status, StatusInProgress)
	}
}
...
func TestPriorityRoundTrip(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			meta := issue.Metadata
			if meta == nil {
				t.Fatal("push: metadata is nil after IssueToTracker")
			}
			wantBP := fmt.Sprintf("%d", tt.beadsPriority)
			gotBP, ok := meta["beads_priority"].(string)
			if !ok {
				t.Fatalf("push: beads_priority not found or wrong type in metadata, got %v (%T)", meta["beads_priority"], meta["beads_priority"])
			}
			if gotBP != wantBP {
