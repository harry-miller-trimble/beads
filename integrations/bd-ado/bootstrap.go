...
func NewBootstrapMatcher(mapper FieldMapper, heuristicMatch bool) *BootstrapMatcher {
...
func (m *BootstrapMatcher) FindMatch(adoItem *TrackerIssue, localIssues []*BeadsIssue) MatchResult {
	// 1. External ref match
	for _, issue := range localIssues {
		if issue.ExternalRef != "" && issue.ExternalRef == adoItem.URL {
			return MatchResult{
				Matched:   true,
				BeadsID:   issue.ID,
...
type BootstrapIndex struct {
	...
	ExternalRefMap map[string]*BeadsIssue
	// SourceSystemMap maps extracted source system ID → issue for O(1) source_system lookups.
	SourceSystemMap map[string]*BeadsIssue
	// TitleMap maps lowercase title → issues for O(1) heuristic candidate lookup.
	TitleMap map[string][]*BeadsIssue
}

// BuildBootstrapIndex pre-indexes a slice of issues for fast matching.
func BuildBootstrapIndex(issues []*BeadsIssue) *BootstrapIndex {
	idx := &BootstrapIndex{
		ExternalRefMap:  make(map[string]*BeadsIssue, len(issues)),
		SourceSystemMap: make(map[string]*BeadsIssue, len(issues)),
		TitleMap:        make(map[string][]*BeadsIssue, len(issues)),
	}
...
func (m *BootstrapMatcher) FindMatchIndexed(adoItem *TrackerIssue, idx *BootstrapIndex) MatchResult {
	if idx == nil {
		return MatchResult{}
	}
