...
- [ADO (Azure DevOps)](#ado-azure-devops)
...
Commands for syncing issues between beads and Azure DevOps work items.

### Configuration

Before using ADO commands, configure your connection:
...
# AZURE_DEVOPS_PAT, AZURE_DEVOPS_ORG, AZURE_DEVOPS_PROJECT, AZURE_DEVOPS_URL
```

See [CONFIG.md](CONFIG.md#example-azure-devops-ado-integration) for full configuration details.
...
# Pull only — import work items from ADO without pushing local changes
bd ado sync --pull-only

# Push only — export local issues to ADO without pulling
bd ado sync --push-only

# Preview what would change without making modifications
bd ado sync --dry-run

# Conflict resolution (mutually exclusive — default is --prefer-newer)
bd ado sync --prefer-local    # On conflict, keep local beads version
bd ado sync --prefer-ado      # On conflict, use Azure DevOps version
bd ado sync --prefer-newer    # On conflict, use most recently updated (default)

# Additional sync options
bd ado sync --bootstrap-match  # Enable heuristic matching for first sync
bd ado sync --no-create        # Pull-only mode: never create new issues in ADO
bd ado sync --reconcile        # Force reconciliation scan for deleted items

# Combine flags
bd ado sync --pull-only --dry-run --json
```
...
| `--no-create` | Pull-only mode: never create issues in ADO |
...
bd ado status --json
# {"org":"myorg","project":"MyProject","has_token":true,"configured":true}
```

### bd ado projects

List Azure DevOps projects accessible with the configured token. Useful for discovering the correct project name.
